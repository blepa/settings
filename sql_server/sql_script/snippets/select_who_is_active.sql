declare 
	...
	--List of desired  columns, in desired order
	--Note that the final  will be the intersection of all enabled features and all 
	--columns in the list. Therefore, only columns associated with enabled features will 
	--actually appear in the . Likewise, removing columns from this list may effectively
	--disable features, even if they are turned on
	--
	--Each element in this list must be one of the valid  column names. Names must be
	--delimited by square brackets. White space, formatting, and additional characters are
	--allowed, as long as the list contains exact matches of delimited valid column names.
	@_column_list VARCHAR(8000) = '[dd%][session_id][sql_text][sql_command][login_name][wait_info][tasks][tran_log%][cpu%][temp%][block%][reads%][writes%][context%][physical%][query_plan][locks][%]',

	--Column(s) by which to sort , optionally with sort directions. 
	...
	--Formats some of the  columns in a more "human readable" form
	--0 disables outfput format
	--1 formats the  for variable-width fonts
	--2 formats the  for fixed-width fonts
	@format_ TINYINT = 1,
...
*/
	...
	BEGIN;
		RAISERROR('Valid values for @format_ are: 0, 1, or 2', 16, 1);
		RETURN;
	END;
	...
	BEGIN;
							...
							(
								...
								(
									...
									(
										...
										(
											t.text, 
											CHARINDEX(' COLUMNS'+CHAR(13)+CHAR(10)+'--------------', t.text) + 32,
											CHARINDEX('*/', t.text, CHARINDEX(' COLUMNS'+CHAR(13)+CHAR(10)+'--------------', t.text) + 32) - (CHARINDEX(' COLUMNS'+CHAR(13)+CHAR(10)+'--------------', t.text) + 32)
										)
									),
									CHAR(9),
									CHAR(255)
		...
		(
			SELECT 
				RTRIM(LTRIM(
					...
					(
						...
						CASE
							WHEN 
								COALESCE(NULLIF(CHARINDEX(CHAR(13) + 'Formatted', @s, number + 1), 0), LEN(@s)) < 
								COALESCE(NULLIF(CHARINDEX(CHAR(13) + CHAR(255) COLLATE Latin1_General_Bin2, @s, number + 1), 0), LEN(@s))
								THEN COALESCE(NULLIF(CHARINDEX(CHAR(13) + 'Formatted', @s, number + 1), 0), LEN(@s)) - number - 1
							ELSE
								COALESCE(NULLIF(CHARINDEX(CHAR(13) + CHAR(255) COLLATE Latin1_General_Bin2, @s, number + 1), 0), LEN(@s)) - number - 1
						END
					)
				)) AS token,
				number,
				COALESCE(NULLIF(CHARINDEX(CHAR(13) + 'Formatted', @s, number + 1), 0), LEN(@s)) AS _group,
				ROW_NUMBER() OVER
				(
					PARTITION BY 
						COALESCE(NULLIF(CHARINDEX(CHAR(13) + 'Formatted', @s, number + 1), 0), LEN(@s))
			...
			FROM numbers
			WHERE
				SUBSTRING(@s, number, 10) = CHAR(13) + 'Formatted'
				OR SUBSTRING(@s, number, 2) = CHAR(13) + CHAR(255) COLLATE Latin1_General_Bin2
		),
		_tokens AS
		(
			SELECT 
				*,
				CASE _group_order
					WHEN 2 THEN MAX(CASE _group_order WHEN 1 THEN token ELSE NULL END) OVER (PARTITION BY _group)
					ELSE ''
		...
		SELECT 
			'|[' +
				...
				(
					@_column_list,
					number + 1,
					CHARINDEX(']', @_column_list, number) - number - 1
				) + '|]' AS token,
			number
		FROM numbers
		WHERE
			SUBSTRING(@_column_list, number, 1) = '['
	),
	ordered_columns AS
	(
		SELECT
	...
	IF COALESCE(RTRIM(@_column_list), '') = ''
	BEGIN;
		RAISERROR('No valid column matches found in @_column_list or no columns remain due to selected options.', 16, 1);
		RETURN;
	END;
	...
	BEGIN;
		...
		IF 
			@get_locks = 1 
			AND @recursion = 1
			AND @_column_list LIKE '%|[locks|]%' ESCAPE '|'
		BEGIN;
			SELECT
				y.resource_type,
				y.database_name,
		...
		SET @sql = 
			...
			'SELECT TOP(@i)
				...
				' +
				CASE
					WHEN @_column_list LIKE '%|[dd hh:mm:ss.mss|]%' ESCAPE '|' THEN 
						'x.elapsed_time '
					ELSE 
						'0 '
				END + 
				...
				CASE
					WHEN
						(
							@_column_list LIKE '%|[dd hh:mm:ss.mss (avg)|]%' ESCAPE '|' OR 
							@_column_list LIKE '%|[avg_elapsed_time|]%' ESCAPE '|'
						)
						AND @recursion = 1
							THEN 
				...
				CASE
					WHEN 
						@_column_list LIKE '%|[physical_io|]%' ESCAPE '|'
						OR @_column_list LIKE '%|[physical_io_delta|]%' ESCAPE '|'
							THEN 
								'x.physical_io '
					ELSE 
						'NULL '
				...
				CASE
					WHEN 
						@_column_list LIKE '%|[reads|]%' ESCAPE '|'
						OR @_column_list LIKE '%|[reads_delta|]%' ESCAPE '|'
							THEN 
								'x.reads '
					ELSE 
						'0 '
				...
				CASE
					WHEN 
						@_column_list LIKE '%|[physical_reads|]%' ESCAPE '|'
						OR @_column_list LIKE '%|[physical_reads_delta|]%' ESCAPE '|'
							THEN 
								'x.physical_reads '
					ELSE 
						'0 '
				...
				CASE
					WHEN 
						@_column_list LIKE '%|[writes|]%' ESCAPE '|'
						OR @_column_list LIKE '%|[writes_delta|]%' ESCAPE '|'
							THEN 
								'x.writes '
					ELSE 
						'0 '
				...
				CASE
					WHEN 
						@_column_list LIKE '%|[tempdb_allocations|]%' ESCAPE '|'
						OR @_column_list LIKE '%|[tempdb_allocations_delta|]%' ESCAPE '|'
							THEN 
				...
				CASE
					WHEN 
						@_column_list LIKE '%|[tempdb_current|]%' ESCAPE '|'
						OR @_column_list LIKE '%|[tempdb_current_delta|]%' ESCAPE '|'
							THEN 
								'x.tempdb_current '
					ELSE 
						'0 '
				...
				CASE
					WHEN 
						@_column_list LIKE '%|[CPU|]%' ESCAPE '|'
						OR @_column_list LIKE '%|[CPU_delta|]%' ESCAPE '|'
							THEN
								'x.CPU '
					ELSE
						'0 '
				...
				CASE
					WHEN 
						@_column_list LIKE '%|[CPU_delta|]%' ESCAPE '|'
						AND @get_task_info = 2
						AND @sys_info = 1
							THEN 
				...
				CASE
					WHEN 
						@_column_list LIKE '%|[context_switches|]%' ESCAPE '|'
						OR @_column_list LIKE '%|[context_switches_delta|]%' ESCAPE '|'
							THEN 
								'x.context_switches '
					ELSE 
						'NULL '
				...
				CASE
					WHEN 
						@_column_list LIKE '%|[used_memory|]%' ESCAPE '|'
						OR @_column_list LIKE '%|[used_memory_delta|]%' ESCAPE '|'
							THEN 
								'x.used_memory '
					ELSE 
						'0 '
				...
				CASE
					WHEN 
						@_column_list LIKE '%|[tasks|]%' ESCAPE '|'
						AND @recursion = 1
							THEN 
								'x.tasks '
					ELSE 
				...
				CASE
					WHEN 
						(
							@_column_list LIKE '%|[status|]%' ESCAPE '|' 
							OR @_column_list LIKE '%|[sql_command|]%' ESCAPE '|'
						)
						AND @recursion = 1
							THEN 
								'x.status '
				...
				CASE
					WHEN 
						@_column_list LIKE '%|[wait_info|]%' ESCAPE '|' 
						AND @recursion = 1
							THEN 
								CASE @get_task_info
									WHEN 2 THEN
				...
				CASE
					WHEN 
						(
							@_column_list LIKE '%|[tran_start_time|]%' ESCAPE '|' 
							OR @_column_list LIKE '%|[tran_log_writes|]%' ESCAPE '|' 
						)
						AND @recursion = 1
							THEN 
								'x.transaction_id '
				...
				CASE
					WHEN 
						@_column_list LIKE '%|[open_tran_count|]%' ESCAPE '|' 
						AND @recursion = 1
							THEN 
								'x.open_tran_count '
					ELSE 
				...
				CASE
					WHEN 
						@_column_list LIKE '%|[sql_text|]%' ESCAPE '|' 
						AND @recursion = 1
							THEN 
								'x.sql_handle '
					ELSE 
				...
				CASE
					WHEN 
						(
							@_column_list LIKE '%|[sql_text|]%' ESCAPE '|' 
							OR @_column_list LIKE '%|[query_plan|]%' ESCAPE '|' 
						)
						AND @recursion = 1
							THEN 
				...
				CASE
					WHEN 
						@_column_list LIKE '%|[query_plan|]%' ESCAPE '|' 
						AND @recursion = 1
							THEN 
								'x.plan_handle '
					ELSE 
				...
				CASE
					WHEN 
						@_column_list LIKE '%|[blocking_session_id|]%' ESCAPE '|' 
						AND @recursion = 1
							THEN 
				...
				CASE
					WHEN 
						@_column_list LIKE '%|[percent_complete|]%' ESCAPE '|'
						AND @recursion = 1
							THEN 
								'x.percent_complete '
					ELSE 
				...
				CASE
					WHEN 
						@_column_list LIKE '%|[host_name|]%' ESCAPE '|' 
						AND @recursion = 1
							THEN 
								'x.host_name '
					ELSE 
				...
				CASE
					WHEN 
						@_column_list LIKE '%|[login_name|]%' ESCAPE '|' 
						AND @recursion = 1
							THEN 
								'x.login_name '
					ELSE 
				...
				CASE
					WHEN 
						@_column_list LIKE '%|[database_name|]%' ESCAPE '|' 
						AND @recursion = 1
							THEN 
				...
				CASE
					WHEN 
						@_column_list LIKE '%|[program_name|]%' ESCAPE '|' 
						AND @recursion = 1
							THEN 
								'x.program_name '
					ELSE 
				...
				CASE
					WHEN
						@_column_list LIKE '%|[additional_info|]%' ESCAPE '|'
						AND @recursion = 1
							THEN
								'(
									SELECT TOP(@i)
										...
										' +
										CASE
											WHEN @_column_list LIKE '%|[program_name|]%' ESCAPE '|' THEN
												'(
													SELECT TOP(1)
				...
				CASE
					WHEN
						@_column_list LIKE '%|[login_time|]%' ESCAPE '|'
						AND @recursion = 1
							THEN
								'x.login_time '
					ELSE 
		...
		IF 
			@recursion = 1
			AND @_column_list LIKE '%|[sql_text|]%' ESCAPE '|'
		BEGIN;
			DECLARE sql_cursor
		...
		IF 
			@get_outer_command = 1 
			AND @recursion = 1
			AND @_column_list LIKE '%|[sql_command|]%' ESCAPE '|'
		BEGIN;
		...
		IF 
			@get_plans >= 1 
			AND @recursion = 1
			AND @_column_list LIKE '%|[query_plan|]%' ESCAPE '|'
		BEGIN;
		...
		IF 
			@find_block_leaders = 1
			AND @recursion = 1
			AND @_column_list LIKE '%|[blocked_session_count|]%' ESCAPE '|'
		BEGIN;
			WITH
			blockers AS
			(
		...
		IF
			@get_task_info = 2
			AND @_column_list LIKE '%|[additional_info|]%' ESCAPE '|'
			AND @recursion = 1
		BEGIN;
		...
		(
			...
			'( ' +
				'SELECT ' +
					...
					END + 'used_memory, ' +
					CASE
						WHEN @_column_list LIKE '%|_delta|]%' ESCAPE '|' THEN
							--physical_io_delta			
							'CASE ' +
								'WHEN ' +
